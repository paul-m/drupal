 ...
 *
 * Rules:
 * - We only ever change drupal/drupal sites.
...
class ComposerizeStatusCommand extends BaseCommand {
 ...
 //     ->exclude(['core'])
    ...
    $io->write('extensions: ' . implode(', ', iterator_to_array($finder->getIterator())));
  ...
  protected function createNewComposerJson() {
    ComposerJsonManipulator::writeObjectToJsonFile(
      $this->getTemplateComposerJson(), $this->rootComposerJsonPath
    );
  ...
  protected function addRequirementsToComposerJson() {
    ...
    ComposerJsonManipulator::writeObjectToJsonFile(
      $root_composer_json, $this->rootComposerJsonPath
    );
  }
  ...
  protected function mergeTemplateGitignore() {
    ...
    foreach ($template_gitignore as $key => $line) {
      $gitignore_entries[] = str_replace(
        '[drupal-root]', $this->drupalRootRelative, $line
      );
    }
    ...
    file_put_contents(
      $root_gitignore_path, implode('', $merged_gitignore)
    );
  ...
  InputInterface $input, DrupalFinder $drupalFinder
  ) {
  ...
  protected function findContribProjects($root_composer_json) {
    $modules_contrib = DrupalInspector::findContribProjects(
        $this->drupalRoot, "modules/contrib", $root_composer_json
    );
    $modules = DrupalInspector::findContribProjects(
        $this->drupalRoot, "modules", $root_composer_json
    );
    $themes = DrupalInspector::findContribProjects(
        $this->drupalRoot, "themes/contrib", $root_composer_json
    );
    $profiles = DrupalInspector::findContribProjects(
        $this->drupalRoot, "profiles/contrib", $root_composer_json
    );
      ...
      if (array_key_exists('patches', $project)) {
        foreach ($project['patches'] as $key => $patch) {
          ...
          $relative_path = $this->fs->makePathRelative(
            $target_filename, $this->getBaseDir()
          );
